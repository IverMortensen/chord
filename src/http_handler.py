...
class HTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/storage/"):
            ...
            try:
                key = self.path.split("/successor/")[1]
                self.get_successor(int(key))
            except (ValueError, IndexError):
                self.send_error(400, "Invalid key format")
    ...
    def get_successor(self, key: int):
        # Find successor
    ...
    def get_storage(self, raw_key: str):
        ...
        # Get the value
        response = requests.get(f"http://{successor}/value/{raw_key}")
    ...
    def put_storage(self, raw_key: str):
        ...
        # Insert value
        response = requests.put(f"http://{successor}/value/{raw_key}", data=value)
